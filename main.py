...
KOREAN_QUERY_KEYWORD = ['이상', '이하', '초과', '미만', '그리고', '또는', '웹프로그래밍', '파이썬', '알고리즘']
...
def sql_query_context_list2sql_query(sql_query_context_list:list[str])->str:
    ...
    for idx, element in enumerate(sql_query_context_list):
        temp_sql_query = ""
        if element in SQL_VALUE_COMPARISION_OPERATOR_KEYWORD and not is_second_compraision_operator:
            is_second_compraision_operator = True
        elif element in SQL_VALUE_COMPARISION_OPERATOR_KEYWORD and is_second_compraision_operator:
...
def korean2sql_query(korean_query:str)->str:
    ...
    return sql_query_context_list2sql_query(korean_context2sql_query_context(asembly_list))


def search_from_number(name_index, score):
    number = input("학번을 입력하세요 : ")
    try:
        print(f"{number} {name_index[number]} 웹 프로그래밍[{score[number][0]}]점 파이썬 프로그래밍[{score[number][1]}]점 알고리즘[{score[number][2]}]점 총점 {sum(score[number][0:2])}점")
    except KeyError:
        print("해당 학생은 존재하지 않습니다.")

def sep_grade(name_index, score, sorted_score, temp_score):
...
def load_data(name_index, score, filename="./data.txt"):
    with open(filename,"r",encoding=UTF_8) as f:
        ...
        for line in lines:
            ...
            score[splited_data[0]] = [int(splited_data[2]),int(splited_data[3]),int(splited_data[4])]
...
def sep_grade_from_number(UTF_8, name_index, score):
    ...
    for year in range(2020,2025):
        number_sep[str(year)] = sorted(number_sep[str(year)],key=lambda x: x[1][3],reverse=True)
        write_number_sep_file(UTF_8, name_index, number_sep,year)

def write_number_sep_file(UTF_8, name_index, number_sep,year):
...
if __name__ == "__main__":
    name_index = {}
    score = {}
    load_data(name_index, score)
    web_programming_avg = calc_avg(score.values(),0)
    python_programming_avg = calc_avg(score.values(),1)
    algorithem_avg = calc_avg(score.values(),2)

    #write score
    sorted_by_number = sorted(score.items(),key=lambda x:x[0])
    sorted_score = dict(sorted(score.items(),key=lambda x:x[1][3],reverse=True))
    temp_score = {}

    #sort from number
    score = sep_grade(name_index, score, sorted_score, temp_score)

    #number file separate logic
    sep_grade_from_number(UTF_8, name_index, score)
    ...
    while True:
        try:
            ...
            if command == "1":
                search_from_number(name_index, score)
            elif command == "2":
                is_pass = False
                print("조건문은 조회만 가능합니다.\n학번기준으로 데이터 오름차순 정렬이 되어있습니다.")
                ...
                if not is_pass:
                    ...
                    if not init_sql_table:
                        ...
                        for number, score in sorted_by_number:
